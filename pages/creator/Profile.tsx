  ...
  if (!user || !user.profile) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center">
        <h2 className="text-2xl font-bold text-slate-900 mb-4">Creator not found</h2>
        <Link to="/">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="p-6 text-center border-b border-slate-100">
                <div className="w-32 h-32 rounded-full border-4 border-white shadow-md mx-auto -mt-20 overflow-hidden bg-white">
                  {user.avatarUrl ? (
                    <img src={user.avatarUrl} alt={profile.displayName} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full bg-brand-100 flex items-center justify-center text-4xl font-bold text-brand-600">
                ...
                <div className="mt-4">
                  <h1 className="text-2xl font-bold text-slate-900 flex items-center justify-center gap-2">
                    ...
                    {profile.verification?.isIdentityVerified && (
                      <CheckCircle className="text-brand-500 fill-brand-100" size={24} />
                    )}
                  </h1>
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {profile.categories.map(cat => (
                    <span key={cat} className="px-3 py-1 bg-slate-100 text-slate-600 text-xs font-semibold rounded-full uppercase tracking-wide">
                      {cat}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-6 flex justify-center space-x-3">
                  {profile.socials.instagram && (
                    <a href={`https://instagram.com/${profile.socials.instagram}`} target="_blank" rel="noreferrer" className="p-2 text-slate-400 hover:text-pink-600 bg-slate-50 rounded-lg transition-colors">
                      <Instagram size={20} />
                    </a>
                  )}
                  {profile.socials.tiktok && (
                    <a href="#" className="p-2 text-slate-400 hover:text-black bg-slate-50 rounded-lg transition-colors">
                  ...
                  {profile.socials.youtube && (
                    <a href={profile.socials.youtube} target="_blank" rel="noreferrer" className="p-2 text-slate-400 hover:text-red-600 bg-slate-50 rounded-lg transition-colors">
                      <Youtube size={20} />
                    </a>
                  )}
                </div>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100">
                ...
                <Button variant="outline" className="w-full bg-white">Message</Button>
              </div>
            </div>
            ...
            {profile.socialStats && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="font-bold text-slate-900 mb-4">Audience Stats</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-slate-50 rounded-lg text-center">
                    <div className="text-xl font-bold text-slate-900">{profile.socialStats.totalFollowers}</div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mt-1">Followers</div>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-lg text-center">
                    <div className="text-xl font-bold text-slate-900">{profile.socialStats.engagementRate}</div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mt-1">Engagement</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
              ...
              {profile.experience.skills.length > 0 && (
                <div className="mt-6 pt-6 border-t border-slate-100">
                  <h3 className="font-bold text-slate-900 mb-3 text-sm uppercase tracking-wide">Skills</h3>
                  <div className="flex flex-wrap gap-2">
                    {profile.experience.skills.map(skill => (
                      <span key={skill} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700 text-sm rounded-lg">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {profile.portfolio.images.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {profile.portfolio.images.map((img, idx) => (
                    <div key={idx} className="group aspect-square rounded-xl overflow-hidden bg-slate-100 relative cursor-pointer">
                ...
                <div className="bg-white rounded-xl p-8 text-center text-slate-500 border border-slate-200 border-dashed">
            ...
            {profile.pricing && profile.pricing.packages && profile.pricing.packages.length > 0 && (
              <div>
                 <h2 className="text-xl font-bold text-slate-900 mb-4">Packages</h2>
                 <div className="space-y-4">
                    {profile.pricing.packages.map((pkg) => (
                      <div key={pkg.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col md:flex-row gap-6 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex flex-col items-end justify-center min-w-[140px] border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
            ...
            {(!profile.pricing?.packages || profile.pricing.packages.length === 0) && profile.pricing && (
               <div className="bg-brand-50 rounded-xl p-6 border border-brand-100">
                  <h3 className="font-bold text-brand-900 mb-2">Estimated Rates</h3>
